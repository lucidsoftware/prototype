...
angular.module('particleApp', ['lucidComponents', 'pressData', 'ngDraggable', 'ngSortable', 'dndLists', 'ngRoute'])
    ...
    ////START MAIN CTRL
    ////////////////
    .controller('mainCtrl', function($scope, $timeout, $rootScope, prototypeData) {
        $rootScope.insertSmartContent = function(content) {
            $rootScope.selectedBlock.text.text = content;
            $rootScope.selectedBlock.edit = true;
        ...
        $scope.loading = function() {
            console.log('loading');
            $rootScope.loadingClass = "loaded"; //change to "loading" if you want loading icon
            $rootScope.pages = prototypeData;
            //$rootScope.pages = [{}];
            $timeout(function() {
    ...
    ////START PAGES CTRL
    ////////////////
    .controller('PagesCtrl', function($scope, $timeout, $rootScope, prototypeData) {
        $rootScope.pages = prototypeData;
        $rootScope.currentPage = prototypeData[0];
    ...
    .directive('editPressText', [function() {
        return {
            restrict: 'AE',
            scope: {
            ...
            replace: true,
            template: '<span class="edit-in-place-wrapper">' +
                '    <span clickr-outside="edit=false">' +
                '       <span class="edit-in-place" ng-show="!edit" ng-dblclick="editText(); $event.stopPropagation();">{{editableText}}</span>' +
                '       <input class="edit-in-place edit-in-place-input" ng-model="editableText" click-outside="edit=false" ng-show="edit" ng-enter="edit=false" placeholder="{{placeholder}}" ng-dblclick="selectInput(); $event.stopPropagation();">' +
                '   </span>' +
                '</span>',
